...
# # --- Local-AI controller (Ollama + Supabase pgvector) ----------------------
# # Served at /controller/* alongside the Gemini /chat below, so both can run
# # during the migration. Import is guarded: a missing Ollama or Supabase
# # config must not stop the existing endpoints from booting.
# try:
#     from Ai_controller.api import router as controller_router

#     app.include_router(controller_router)
#     logger.info("Controller pipeline mounted at /controller")
# except Exception as exc:  # pragma: no cover - optional subsystem
#     logger.error("Controller pipeline NOT mounted, /controller/chat will 404: %s", exc)
