...
import { Search, ChevronLeft, ChevronRight, LogOut, User, Briefcase, Clock, ChevronDown, ChevronUp } from "lucide-react";
...
export function CandidatePortal() {
  ...
  const loadCandidateData = async (candidateEmail: string) => {
    ...
    const { data: applications, error: appError } = await supabase
      .from("candidates")
      .select("id, status, applied_at, job_id, jobs(job_title, status)")
  ...
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-100 rounded-md"
              >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {activeTab === "applications" ? (
          <>
            <h2 className="text-2xl font-semibold text-gray-900 mb-8">Riwayat Lamaran</h2>

            {/* Search */}
            <div className="mt-8 flex items-center gap-3 border-b border-gray-300 py-3">
                    ...
                    <div key={r.id} className="rounded-lg border border-border p-5">
                      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <span className="text-xs font-medium text-ink/70">{r.status}</span>
                          <button
                            onClick={() => setExpandedId(isExpanded ? null : r.id)}
                            className="flex items-center gap-1 text-sm font-semibold text-ink underline decoration-brand decoration-2 underline-offset-4"
                          >
                            Lihat progres
                      ...
                      {isExpanded && (
                        <div className="mt-5 border-t border-border pt-4">
                          {history.length === 0 ? (
                            <p className="text-xs text-muted-foreground">Belum ada riwayat tahapan.</p>
                          ) : (
                              ...
                              {history.map((h, idx) => (
                                <li key={h.id} className="flex gap-3">
                                  <div className="flex flex-col items-center">
                                    <span
                                      className={cn(
                                        ...
                                        idx === history.length - 1 ? "bg-brand" : "bg-border",
                                      )}
                                    />
                                  ...
                                  <div className="pb-2">
                                    ...
                                    {h.notes && (
                                      <p className="mt-1 text-xs text-ink/70">{h.notes}</p>
                                    )}
                                  </div>
                                </li>
                              ))}
            ...
            {totalPages > 1 && (
              <div className="mt-8 flex items-center justify-center gap-4">
                <button
                  ...
                  className="inline-flex size-9 items-center justify-center rounded-full border border-border disabled:opacity-30"
                >
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-lg shadow border p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className="mt-1 text-lg text-gray-900">{profile.mobile_phone_wa || "Not provided"}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Member since</label>
                  <p className="mt-1 text-lg text-gray-900">{fmtDate(profile.created_at)}</p>
                </div>
              </div>
            ) : (
  ...
  return (
    <label className="relative inline-flex items-center">
      <select
        ...
        className={cn(
          "cursor-pointer appearance-none rounded-full border border-gray-300 bg-white py-2 pl-4 pr-8 text-sm font-medium outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500",
          value ? "border-blue-500 text-gray-900" : "text-gray-500",
        )}
      >
